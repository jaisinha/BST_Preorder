...
void preordder(Node *node)
{
    if (node == NULL)
        return;
    cout << node->val << " ";
    preordder(node->left);
    preordder(node->right);
}
Node *buildBST(int preoder[], int bound, int n)
{
    static int idx = 0;
    if (preoder[idx] > bound || idx == n)
        return NULL;
    Node *node = new Node(preoder[idx++]);
    node->left = buildBST(preoder, node->val, n);
    node->right = buildBST(preoder, bound, n);
    return node;
}
int main()
{
    int preorder[] = {1, 2, 3};
    int inorder[] = {2, 1, 3};
    Node *root = buildBST(preorder, INT_MAX, 3);
    preordder(root);
}
